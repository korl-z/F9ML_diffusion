...
class DensityRatioAnalysis:
    ...
    def get_density_ratio(self, data):
        """
        density ratio r(x) = P_ML(x) / P_MC(x) from C2ST
        r(x) = exp(sigma^{-1}[p(y=1|x)])
        """
    ...
    def plot_density_ratio_distribution(
        self,
        mc_data,
        ml_data,
        tail_cut_low=0.971,
        tail_cut_high=1.0125,  # eyeballing it for now
        save_path=None,
        save_data_path=None,
    ):
        """
        ...
        mc_tail_frac = np.sum((r_x_mc < tail_cut_low) | (r_x_mc > tail_cut_high)) / len(
            r_x_mc
        )  # percentages of data in tails
        ml_tail_frac = np.sum((r_x_ml < tail_cut_low) | (r_x_ml > tail_cut_high)) / len(
            r_x_ml
        )

        print(
            f"perc. in tails: MC: {mc_tail_frac*100:.2f}%, ML: {ml_tail_frac*100:.2f}%"
        )

        bin_range = (0.94, 1.06)  # eyeball it
        ...
        # save data for plotting locally, since this pc does not want to cooperate
        if save_data_path:
            df = pd.DataFrame(
                {
            ...
            metadata = {
                ...
                "mc_extreme_count": int(np.sum((r_x_mc < 0.7) | (r_x_mc > 1.3))),
                "ml_extreme_count": int(np.sum((r_x_ml < 0.7) | (r_x_ml > 1.3))),
            }
    ...
    def get_tail_events(self, data, r_x, tail_cut_low=0.94, tail_cut_high=1.08):
        ...
        print(
            f"Tail events: {np.sum(tail_mask)} ({np.sum(tail_mask)/len(data)*100:.2f}%)"
        )
...
if __name__ == "__main__":
    registered_model_name = (
        "BinaryClassifier_unet1d_ddpm_model_c2st_gen_model_all"  # either/or
    )
    # registered_model_name = "BinaryClassifier_unet1D_EDM_s_model_c2st_gen_model_all"  #either/or
    ...
    npy_proc = HIGGSNpyProcessor(
        ...
        hold_mode=True,
        use_hold=True,  # holdout/test set
    )
    ...
    # generated data
    ml_test_data = np.load(
        "/data0/korlz/f9-ml/ml/data/HIGGS/HIGGS_generated_unet1d_ddpm_model6_6.npy"
    )
