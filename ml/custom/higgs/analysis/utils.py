...
def run_chainer(
    ...
    with open(f"/data0/korlz/f9-ml/ml/custom/higgs/config/{model_type}/data_config.yaml", "r") as file:
        ...
        for ver, (model_name, model) in tqdm(
            ...
            for i in tqdm(range(resample), leave=False, desc=f"Sampling model {model_name}"):
                ...
                if resample == 1 or i == 0:
                    npy_file = f"/data0/korlz/f9-ml/ml/data/HIGGS/HIGGS_generated_{model_name}{ver}.npy"
                else:
                    npy_file = f"/data0/korlz/f9-ml/ml/data/HIGGS/HIGGS_generated_{model_name}{ver}_{i}.npy"
