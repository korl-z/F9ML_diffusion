...
class FlowCut(Cut):
    ...
    def plot_density_cut(
        ...
        if use_sigmoid:
            plt.savefig(f"{self.save_dir}/EDM_density_sig_bkg_sigmoid.pdf")
        else:
            plt.savefig(f"{self.save_dir}/EDM_density_sig_bkg.pdf")
    ...
    def plot_after_cut(self, n_bins=70):
        colors = ["C2", "C0", "C1"]
        return super().plot_after_cut(n_bins, legend=["ML generated", "MC bkg", "MC sig"], postfix="_EDM", cs=colors)

    def plot_roc_acc(self, cut_threshold=0.5, M=-1):
        return super().plot_roc_acc(self.density_dct, cut_threshold, M, postfix="_EDM", use_sigmoid=True)
...
if __name__ == "__main__":
    ...
    classifier_model = "BinaryClassifier_sigbkg_gauss_rank_best2/1"
