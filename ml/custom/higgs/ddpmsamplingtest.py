...
@hydra.main(config_path="config/ddpm/", config_name="main_config", version_base=None)
def main(cfg):
    setup_logger()
    ...
    file_dir, file_name = get_generated_data(
        select_model=model_name, N=N, chunks=1000, ver=ver
    )
    ...
    for feat_idx in range(D):
        ...
        # ax.plot(X, scipy.stats.norm.pdf(X, 0, 1), '-k', lw=2)
        add_data_mc_ratio(
            ax=ax,
            bin_edges=bin_edges,
    ...
    for j in range(D, len(ax_flat)):
        ax_flat[j].axis("off")
    plt.tight_layout()
    output_filename = f"analysis/plots/testplots/{model_name}_{ver}_comparison_plot.pdf"
