...
def main(cfg):
    ...
    for feat_idx in range(D):
        ax = ax_flat[feat_idx]
        bin_edges = np.histogram_bin_edges(inv_gen[:, feat_idx], bins=50)
        ...
        ax.hist(inv_gen[:, feat_idx], bins=bin_edges, density=True, histtype="step", lw=1.0, color="blue", label="gen")
        ax.set_yscale("log")
        ax.legend()
