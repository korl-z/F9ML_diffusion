...
class TwoSampleBuilder:
    ...
    def _check_valid(self, data_X, data_Y, selection_X, selection_Y):
        valid_mask_X = ~np.isnan(data_X).any(axis=1) & ~np.isinf(data_X).any(axis=1)
        valid_mask_Y = ~np.isnan(data_Y).any(axis=1) & ~np.isinf(data_Y).any(axis=1)
        combined_valid_mask = valid_mask_X & valid_mask_Y

        original_len = data_X.shape[0]
        data_X = data_X[combined_valid_mask]
        data_Y = data_Y[combined_valid_mask]
        
        if data_X.shape[0] < original_len:
            logging.warning(
                f"NaNs or infs found! Kept {data_X.shape[0]} valid rows out of {original_len}."
            )

        if data_X.shape[0] == 0:
            raise ValueError("After NaN/inf filtering, no valid data remains. Check your preprocessing steps.")
