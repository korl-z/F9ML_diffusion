...
def register_from_checkpoint(trainer, base_module: L.LightningModule, model_name=None, save_module=True):
    ...
    This version uses the standard MLflow logging pattern which is platform-independent.
    """
    logger = trainer.logger
    ...
    if hasattr(base_module, 'model_ema') and base_module.model_ema is not None:
        ...
        logging.info("No EMA model found. Registering from best checkpoint.")
        ...
        if callback and callback.best_model_path:
            ...
            checkpoint = torch.load(callback.best_model_path, map_location=base_module.device, weights_only=False)
            # Load the state_dict into the existing base_module
            base_module.load_state_dict(checkpoint['state_dict'])
            model_to_save = base_module.model
        else:
            logging.warning("No best checkpoint path found. Saving the model's final weights.")
    ...
    if logger.save_dir and os.path.exists(logger.save_dir):
        run_dir = os.path.join(logger.save_dir, logger.experiment_id, logger.run_id)
        checkpoints_dir = os.path.join(run_dir, "checkpoints")
        if os.path.exists(checkpoints_dir):
            try:
                shutil.rmtree(checkpoints_dir)
                logging.info(f"Removed local checkpoint directory: {checkpoints_dir}")
            except Exception as e:
                logging.warning(f"Failed to remove {checkpoints_dir}: {e}")
