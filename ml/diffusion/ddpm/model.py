...
class DDPMPrecond(nn.Module):
    """
    A wrapper for the DDPM network.
    """
    ...
    def set_diffuser(self, diffuser):
        """Attach a Diffuser instanc."""
        self.diffuser = diffuser
        return self

    def forward(self, x: torch.Tensor, t: torch.Tensor):
        """
        x: [B, D] - input data ([B, 18])
        t: [B] - int timesteps 0..T-1
        returns: eps_pred [B, D] - pred noise, flattened
        """
...
class iDDPMPrecond(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        x: [B, D]
        t: [B] ints 0..T-1
        returns: A single tensor [B, 2*D] = [B, (eps_pred, v_pred)].
        """
    ...
    def sample(self, num_samples: int, chunks: int = 20):
        """
        method for generating in chunks.
        """
        device = next(self.parameters()).device
        self.eval()
...
class iDDPM2Precond(nn.Module):
    ...
    def _run_sampler(self, latents, sampler_cfg):
        ...
        j_steps = torch.linspace(
            j_max, j_min, num_steps + 1, device=latents.device
        ).long()
