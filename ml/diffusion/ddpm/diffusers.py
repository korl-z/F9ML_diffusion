...
class DiffuserDDPM:
    """
    Forward diffusion for DDPM: data (shape [B, D], D = #features).
    """
...
class DiffuserDDPMeps:
    """
    Forward noise schedule and q-sampling operations.
    - timesteps: T
    - beta_schedule: 'linear' / pass a tensor of betas
    """
    ...
    def __init__(
        ...
        if betas is not None:
            ...
            self.betas = make_beta_schedule(
                scheduler, self.T, beta_start, beta_end, device=self.device
            )
...
class DiffuseriDDPMeps:
    def __init__(
        ...
        self.posterior_mean_coef1 = (
            self.betas * torch.sqrt(self.alpha_cum_prev) / (1.0 - self.alpha_cum)
        )
        self.posterior_mean_coef2 = (
            (1.0 - self.alpha_cum_prev)
            * torch.sqrt(self.alphas)
            / (1.0 - self.alpha_cum)
        )
        self.posterior_variance = (
            self.betas * (1.0 - self.alpha_cum_prev) / (1.0 - self.alpha_cum)
        )
