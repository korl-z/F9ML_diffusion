...
class EDMModule(Module):
    ...
    def __init__(
        ...
        # estimate data_sigma from train dataset (should be \approx 1 if scaled correctly)
        self.datamodule.setup(stage="fit")
        train_data = self.datamodule.train.X.numpy()
        self.sigma_data = train_data.std()
        logging.info(f"[EDMModule] sigma_data from training set: {self.sigma_data:.4f}") #sanity check 
...
class VPModule(Module):
    ...
    def __init__(
        ...
        # wrapper model, always the same
        self.model = VPPrecond(model, img_shape=self.IMG_SHAPE, M=self.model_conf["M"])
...
class VEModule(Module):
    ...
    def __init__(
        ...
        # wrapper model, always the same
        self.model = VEPrecond(model, img_shape=self.IMG_SHAPE)
